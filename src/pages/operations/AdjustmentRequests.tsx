...
async function fetchRequests(status: StatusFilter): Promise<AdjustmentRequest[]> {
    ...
    for (const r of requests) {
        const adjItems = await pb.collection("stock_adjustment_items").getFullList({
            filter: `request_id = "${r.id}"`,
            expand: "product_id",
        })
        const mappedItems: RequestItem[] = adjItems.map((it) => {
...
export default function AdjustmentRequests() {
    ...
    const load = async (status: StatusFilter) => {
        ...
        try {
            ...
            if (msg.includes("Missing collection") || error?.status === 404) {
                toast.error("stock_adjustment_requests collection not found. Run scripts/setup-pocketbase.js.")
            } else {
    ...
    const handleApprove = async () => {
        if (!actionId || !profile) return
        if (!isAdmin) {
            toast.error("Only admins can approve adjustments")
            return
        }
    ...
    const handleReject = async () => {
        if (!actionId || !profile) return
        if (!isAdmin) {
            toast.error("Only admins can reject adjustments")
            return
        }
        setActionLoading(true)
        try {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-3xl font-bold tracking-tight">Adjustment Requests</h2>
                <p className="text-muted-foreground">
                    {isAdmin
                        ? "Review and approve pending adjustments. Stock changes only after approval."
                        : "Track adjustment requests. Only admins can approve — stock changes after approval."}
                </p>
            </div>
                                    ...
                                    <TableRow key={req.id} className="hover:bg-muted/50">
                                        <TableCell>
                                            <Button variant="ghost" size="icon" onClick={() => toggleRow(req.id)}>
                                                {expanded.has(req.id) ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                                            </Button>
                                        </TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell>
                                            <Badge
                                                ...
                                                className={cn(req.direction === "increase" && "bg-green-100 text-green-800 border-green-200")}
                                            >
                                                {req.direction === "increase" ? (
                                                    <span className="flex items-center gap-1"><ArrowUp className="h-3 w-3" /> Increase</span>
                                                ) : (
                                                    <span className="flex items-center gap-1"><ArrowDown className="h-3 w-3" /> Decrease</span>
                                                )}
                                            </Badge>
                                        </TableCell>
                                        <TableCell>
                                            <div className="text-sm font-medium">{req.items.length} product(s)</div>
                                            <div className="text-xs text-muted-foreground">Total qty: {req.totalQty.toLocaleString()}</div>
                                        </TableCell>
                                            ...
                                            {req.status === "pending" && isAdmin ? (
                                                <div className="flex justify-end gap-2">
                                                    <Button size="sm" className="bg-green-700 hover:bg-green-800"
                                                        onClick={() => { setActionId(req.id); setConfirmApproveOpen(true) }}>
                                                        <Check className="h-4 w-4 mr-1" /> Approve
                                                    </Button>
                                                    <Button size="sm" variant="destructive"
                                                        onClick={() => { setActionId(req.id); setRejectReason(""); setRejectOpen(true) }}>
                                                        <X className="h-4 w-4 mr-1" /> Cancel
                                                    </Button>
                                                </div>
                                            ) : (
                                                <span className="text-xs text-muted-foreground">
                                                    {req.status === "pending" ? "Awaiting admin approval" : req.reviewedBy ? `By ${req.reviewedBy}` : "—"}
                                                </span>
                                            )}
                                        </TableCell>
                                    </TableRow>,
                                                                ...
                                                                {req.items.map((item) => (
                                                                    <TableRow key={item.id}>
                                                                        ...
                                                                        <TableCell className="text-right font-mono font-bold">
                                                                            <span className={req.direction === "increase" ? "text-green-600" : "text-red-600"}>
                                                                                {req.direction === "increase" ? "+" : "-"}{item.quantity}
                                                                            </span>
                                                                        </TableCell>
                                                                    </TableRow>
                                                                ))}
            ...
            <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            The request will be marked as rejected. Stock will not change.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setRejectOpen(false)} disabled={actionLoading}>Back</Button>
